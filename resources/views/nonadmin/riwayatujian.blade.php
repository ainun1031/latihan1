...
<html lang="en">
	...
	<body class="no-skin" onLoad="viewdata_riwayatujian()">
				...
				<div class="main-content-inner">
					...
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li class="active">Riwayat Ujian</li>
						</ul><!-- /.breadcrumb -->
...
<script>
	...
	function viewdata_riwayatujian()
	{
		$.ajaxSetup({headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
		$.ajax
		({
			type:'GET',
			url:'{{ route("nonadmincontroller.viewdata_riwayatujian") }}',
			...
			success:function(xmlHttp)
			{
				...
				if(len > 0)
				{
					...
					vhtml += "<table class='table table-striped table-bordered table-hover'>";
						vhtml += "<thead>";
							vhtml += "<tr>";
								vhtml += "<th>ID Riwayat</th>";
								vhtml += "<th>ID Ujian</th>";
								vhtml += "<th>Nama Ujian</th>";
								vhtml += "<th>Informasi Ujian</th>";
								vhtml += "<th>Maximal Attempt</th>";
								vhtml += "<th>Batas Waktu</th>";
								vhtml += "<th>Tanggal Attempt</th>";
								vhtml += "<th>Jam Attempt</th>";
								vhtml += "<th>Total Nilai</th>";
								vhtml += "<th>Aksi</th>";
							vhtml += "</tr>";
						vhtml += "</thead>";
						vhtml += "<tbody>";
						...
						{
							var data_arr_retbuf = arr_retbuf[i].split('||');   //$v_idriwayat . '||' . $v_idujian . '||' . $v_namaujian . '||' . $v_informasiujian . '||' . $v_maximalattempt . '||' . $v_bataswaktu . '||' . $v_tanggalattempt . '||' . $v_jamattempt . '||' . $v_iduser . '||' . $v_namalengkap . '||' . $v_email . '||' . $v_hp . '||' . $v_urlprofilefacebook . '||' . $v_urlprofileinstagram . '||' . $v_urlprofiletwitter . '||' . $v_urlprofileyoutube . '||' . $v_tempattinggal . '###';
							var v_idriwayatujian = data_arr_retbuf[0];
							var v_idujian = data_arr_retbuf[1];
							var v_namaujian = data_arr_retbuf[2];
							var v_informasiujian = data_arr_retbuf[3];
							var v_maximalattempt = data_arr_retbuf[4];
							var v_bataswaktu = data_arr_retbuf[5];
							var v_tanggalattempt = data_arr_retbuf[6];
							var v_jamattempt = data_arr_retbuf[7];
							var v_iduser= data_arr_retbuf[8];
							var v_namalengkap= data_arr_retbuf[9];
							var v_email = data_arr_retbuf[10];
							var v_hp = data_arr_retbuf[11];
							var v_urlprofilefacebook = data_arr_retbuf[12];
							var v_urlprofileinstagram = data_arr_retbuf[13];
							var v_urlprofiletwitter = data_arr_retbuf[14];
							var v_urlprofileyoutube = data_arr_retbuf[15];
							var v_tempattinggal = data_arr_retbuf[16];
							vhtml += "<tr>";
								vhtml += "<th>"+v_idriwayatujian+"</th>";
								...
								vhtml += "<th>"+v_tanggalattempt+"</th>";
								vhtml += "<th>"+v_jamattempt+"</th>";
								vhtml += "<th>(under construction)</th>";
								vhtml += "<th><a style='cursor:pointer' onclick='do_lanjutmengerjakan("+v_idriwayatujian+","+v_idujian+")'>Lanjut Mengerjakan</a></th>";
							vhtml += "</tr>";
						}
						vhtml += "</tbody>";
					vhtml += "</table>";
					document.getElementById("div_riwayatujian").innerHTML = vhtml;
				}
			}
		});
	}
	...
	function do_lanjutmengerjakan(id_riwayatujian,id_ujian)
	{
		//messagebox
		var vbox1 = confirm("Yes=Lanjut Mengerjakan");

		//if yes (get data)
		if(vbox1 == true)
		{
			var v_idriwayatujian;
			var v_idujian;
			var retbuf;
			
			v_idriwayatujian = id_riwayatujian;
			v_idujian = id_ujian;
		}
		
		//if yes (cek_sisawaktusebelumlanjutmengerjakan)
		if(vbox1 == true)
		{
			//cek_sisawaktusebelumlanjutmengerjakan(id_riwayatujian,id_ujian);
			$.ajaxSetup({headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
			$.ajax
			({
				type:'GET',
				url:'{{ route("nonadmincontroller.cek_sisawaktusebelumlanjutmengerjakan") }}',
				data: {post_idriwayatujian:v_idriwayatujian,post_idujian:v_idujian},
				success:function(xmlHttp)
				{
					...
					if(retbuf=="stop")
					{
						...
						message_content += "<br>";
						message_content += "<h2>Ujian telah berakhir. <br> Harap hubungi Admin.</h2>";
						message_content += "</center>";
		...
		if(vbox1 == true)
		{
			window.location = "{{ url('/nonadmin/view_ujian') }}?id_ujian="+v_idujian+"&id_attempt="+v_idriwayatujian;
		}
	}
</script>